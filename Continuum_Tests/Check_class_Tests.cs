...
namespace Continuum_Tests
{
    ...
    public class Check_class_Tests
    {
        ...
        public void TopoCheck_Test()
        {
            ...
            bool isOk = check.TopoCheck(thisInst.topo, 850000, 4500000, "Test 1", allOrPlot);
            Assert.AreEqual(isOk, true, "Wrong TopoCheck Test 1");

            isOk = check.TopoCheck(thisInst.topo, 684480, 4663890, "Test 2", allOrPlot);
            Assert.AreEqual(isOk, false, "Wrong TopoCheck Test 2");

            isOk = check.TopoCheck(thisInst.topo, 1053800, 4350000, "Test 3", allOrPlot);
            Assert.AreEqual(isOk, false, "Wrong TopoCheck Test 3");

            isOk = check.TopoCheck(thisInst.topo, 688300, 4335000, "Test 4", allOrPlot);
            Assert.AreEqual(isOk, true, "Wrong TopoCheck Test 4");

            isOk = check.TopoCheck(thisInst.topo, 1034000, 4677000, "Test 5", allOrPlot);
        ...
        public void LandCoverCheck_Test()
        {
            ...
            bool isOk = check.LandCoverCheck(thisInst.topo, 682000, 4709000, "Test 1", allOrPlot);
            Assert.AreEqual(isOk, true, "Wrong TopoCheck Test 1");

            isOk = check.LandCoverCheck(thisInst.topo, 682000, 4716600, "Test 2", allOrPlot);
            Assert.AreEqual(isOk, false, "Wrong TopoCheck Test 2");

            isOk = check.LandCoverCheck(thisInst.topo, 1077700, 4674500, "Test 3", allOrPlot);
            Assert.AreEqual(isOk, true, "Wrong TopoCheck Test 3");

            isOk = check.LandCoverCheck(thisInst.topo, 1083650, 4674000, "Test 4", allOrPlot);
